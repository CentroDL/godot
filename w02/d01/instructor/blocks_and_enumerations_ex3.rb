...
# 3. Print out just the students whose names start with vowels.
#

students.each { |student| $stdout.puts(student) }

students.each { |student| $stdout.puts(student.chars.first.upcase + ".") }

students.each do |student|
  $stdout.puts(student) if ["A","E","I","O","U"].include?(student.chars.first.upcase)
end
...
klass.each { |student| $stdout.puts(student[:name]) }

klass.each { |student| $stdout.puts(student[:name].split.last) }

github_names = []
klass.each do |student|
  github_names << student[:github]
end
